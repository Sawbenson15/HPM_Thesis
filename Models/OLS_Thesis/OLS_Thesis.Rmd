...
# 2. Conditional Mean: City vs Rural
library(plyr)
...
# Conditional Mean: Rural pre vs post corona
library(plyr)
city_limits_mean_data <- ddply(subset(data_factor, data_factor$city_limits == 0 ), "infections_period", summarise, city_limits_mean = mean(sold_price, na.rm = TRUE))
...
ggplot(data = subset(data_factor, data_factor$city_limits == 0), aes(x = sold_price)) +
    geom_density(alpha = 0.5, position = "identity", fill = very_low) +
...
# Distribution: Infection: Rural
ggplot(data = subset(data_factor, data_factor$city_limits == 0), aes(x = sold_price, fill = infections_period)) +
    geom_density(alpha = 0.5, position = "identity") +
...
# Testing Corona, Rural vs City General
...
# Add 3mma
data_factor_core_clean$infections_3mma <- data_factor$infections_3mma

# Add city limits
data_factor_core_clean$city_limits <- data_factor$city_limits

# split by city limit
data_city_test <- subset(data_factor_core_clean, data_factor_core_clean$city_limits == 1)

# Remove city limit
data_city_test <- subset(data_city_test, select = -c(city_limits)) 


lm_corona_city_gen <- lm(sold_price ~ . 
                       ...
                       ,data_city_test)
coeftest(lm_corona_city_gen, vcov = vcovHC(lm_corona_city_gen, method = "White2", type = "HC0"))

# Table output
tab_model(lm_corona_city_gen, 
          auto.label = TRUE, 
          collapse.ci = TRUE,
          terms = c("(Intercept)",
                    "infections_3mma"))
...
# split by city limit
data_city_test <- subset(data_factor_core_clean, data_factor_core_clean$city_limits == 0)

# Remove city limit
data_city_test <- subset(data_city_test, select = -c(city_limits)) 

lm_corona_city_rural <- lm(sold_price ~ . 
...
# Table output
tab_model(lm_corona_city_rural, 
          auto.label = TRUE, 
          collapse.ci = TRUE,
          terms = c("(Intercept)",
                    "infections_3mma"))
...
ggplot(data_index, aes(x = Date)) +
    ...
    labs(title = "Louisiana Housing Index",
         caption = "Weighted Average Price per Sqft.") 
...
ggplot(data_index_gdp, aes(x = date)) + 
    geom_line(aes(y = real_gdp_Index), color = very_low, linetype = "dashed", size = .5) +
    geom_line(aes(y = real_gdp_re_specific_index), color = med, size = .5) +
...
# Nominal
ggplot() + 
    geom_line(data_index_fred_1975_total,
              mapping = aes(x = date, 
                            y = gdp_pc_nom_index_1975), 
                            color = low, 
                            linetype = "dashed", 
                            size = .5) +
    geom_line(data_index_fred_1975_total,
              mapping = aes(x = date, y = re_nom_index_1975), 
              color = med, size = .5) +
    theme(legend.position = "bottom" ) +
    xlab(" ") +
    ylab("Index Value (1975 Q1 = 100)") +
    labs(title = "U.S. GDP and Housing Index",
         caption = "FRED, St. Louis: GDP in Blue, Real Estate index in Green") +
...
p1 <- ggdraw() + draw_image("/Users/sawyerbenson/Documents/Master Thesis/HPM_Thesis/Writing & Literature/Graphics from pptx/Results/City/dist_city.png", scale = 0.985)

p2 <- ggdraw() + draw_image("/Users/sawyerbenson/Documents/Master Thesis/HPM_Thesis/Writing & Literature/Graphics from pptx/Results/City/dist_city_corona.png", scale = 1)

p3 <- ggdraw() + draw_image("/Users/sawyerbenson/Documents/Master Thesis/HPM_Thesis/Writing & Literature/Graphics from pptx/Results/City/dist_rural.png", scale = 0.90)

p4 <- ggdraw() + draw_image("/Users/sawyerbenson/Documents/Master Thesis/HPM_Thesis/Writing & Literature/Graphics from pptx/Results/City/dist_rural_corona.png", scale = 0.90)
